...
public class EmailService {
    ...
    // ========================================================================================
    // ⚠️ ATENCIÓN: DEBES PEGAR TU API KEY DE BREVO AQUÍ PARA QUE LOS CORREOS LLEGUEN
    // Consíguela gratis en: https://www.brevo.com/es/ (Sección SMTP & API)
    // ========================================================================================
    private static final String BREVO_API_KEY = "PEGA_AQUI_TU_API_KEY_DE_BREVO"; 
    
    private static final String SENDER_EMAIL = "angeldanicp@gmail.com";
    private static final String SENDER_NAME = "Finca La Rosa";
    ...
    public static void enviarCodigoOTP(String destinatario, String codigo) throws Exception {
        ...
        // Construir el JSON para la API de Brevo usando Gson
        JsonObject senderObj = new JsonObject();
        senderObj.addProperty("name", SENDER_NAME);
        senderObj.addProperty("email", SENDER_EMAIL);

        JsonObject toObj = new JsonObject();
        toObj.addProperty("email", destinatario);
        
        JsonArray toArray = new JsonArray();
        toArray.add(toObj);

        JsonObject requestBody = new JsonObject();
        requestBody.add("sender", senderObj);
        requestBody.add("to", toArray);
        requestBody.addProperty("subject", "Código de Seguridad - Finca La Rosa");
        requestBody.addProperty("htmlContent", htmlContent);

        // Enviar la petición HTTP POST a Brevo (Bypass de puertos bloqueados en Railway)
        ...
        if (response.statusCode() != 201) {
            throw new Exception("Error HTTP " + response.statusCode() + " al enviar correo con Brevo. Asegúrate de haber puesto tu API Key en EmailService.java. Respuesta: " + response.body());
        }
    }
}
